...
class TestTensorflowModels(tests.python.base_test.PSLTest):
    ...
    def test_sign_model(self):
        ...
        batch_complete, pre_train_results = sign_model.internal_eval(test_data, options=options)
        sign_model.internal_fit(train_data, None, options=options)
        batch_complete, post_train_results = sign_model.internal_eval(test_data, options=options)
        ...
        with tempfile.TemporaryDirectory(suffix = '_TestNeuPSL') as temp_dir:
            ...
            batch_complete, post_load_results = saved_sign_model.internal_eval(test_data, options=options)
