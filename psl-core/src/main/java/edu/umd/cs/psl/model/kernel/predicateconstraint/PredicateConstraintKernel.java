...
public class PredicateConstraintKernel implements Kernel {
	...
	public PredicateConstraintKernel(StandardPredicate p, PredicateConstraintType t) {
		Preconditions.checkArgument(p.getNumberOfValues()==1,"Predicate Constraints are only supported on single valued predicates!");
		Preconditions.checkArgument(p.getArity()==2,"Currently, PredicateConstraints only support binary predicates!");
		constraintType = t;
		predicate = p;
		
		hashcode = new HashCodeBuilder().append(predicate).append(constraintType).toHashCode();
	}
	...
	public void setParameters(Parameters para) {
		throw new UnsupportedOperationException("This evidence type does not have parameters!");
	}
	...
	public void notifyAtomEvent(AtomEvent event, Atom atom, GroundingMode mode, ModelApplication app) {
		if (AtomEventSets.IntroducedInferenceAtom.subsumes(event)) {
			if (atom.getRegisteredGroundKernels(this).isEmpty()) {
				...
				PredicateConstraint con = new PredicateConstraint(this,anchor);
				
				GroundKernel oldcon = app.getGroundKernel(con);
				if (oldcon!=null) {
					((PredicateConstraint)oldcon).addAtom(atom);
					...
					ResultList res = app.getDatabase().query(query, ImmutableList.of(var));
					for (int i=0;i<res.size();i++) {
						GroundTerm[] terms = new GroundTerm[2];
						terms[pos]=anchor;
						terms[1-pos]=res.get(i)[0];
						if (!terms[1-pos].equals(other)) {
							con.addAtom(app.getAtomManager().getAtom(predicate, terms));
						}
					}
			...
			throw new UnsupportedOperationException("Currently, only insertions are supported!");
		}
	}
	...
	public boolean equals(Object oth) {
		...
		return predicate.equals(con.predicate) && constraintType==con.constraintType;
	}
