...
public abstract class HyperplaneTerm extends ADMMObjectiveTerm {
	...
	protected void project() {
		if (x.length == 1) {
			...
			x[0] = reasoner.getStepSize() * reasoner.getConsensusValue(zIndices[0]) - y[0];
			x[0] -= reasoner.getStepSize() * coeffs[0] / coeffs[1] * (-1.0 * constant / coeffs[1] + reasoner.getConsensusValue(zIndices[1]) - y[1] / reasoner.getStepSize());
			x[0] /= reasoner.getStepSize() * (1.0 + coeffs[0] * coeffs[0] / coeffs[1] / coeffs[1]);
			
			x[1] = (constant - coeffs[0] * x[0]) / coeffs[1];
		} else {
			double[] point = new double[x.length];
			for (int i = 0; i < x.length; i++)
				point[i] = reasoner.getConsensusValue(zIndices[i]) - y[i] / reasoner.getStepSize();
			
			/* For point (constant / coeffs[0], 0,...) in hyperplane dotted with unitNormal */
			double multiplier = -1.0 * constant / coeffs[0] * unitNormal[0];
