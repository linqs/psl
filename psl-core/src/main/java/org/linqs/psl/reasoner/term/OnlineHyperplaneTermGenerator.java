 ...
 */
public abstract class OnlineHyperplaneTermGenerator<T extends ReasonerTerm, V extends ReasonerLocalVariable> implements TermGenerator<T, V> {
    private static final Logger log = LoggerFactory.getLogger(OnlineHyperplaneTermGenerator.class);

    public static final String CONFIG_PREFIX = "onlinehyperplanetermgenerator";

    private boolean invertNegativeWeight;

    public OnlineHyperplaneTermGenerator() {
    ...
    public T createTerm(GroundRule groundRule, TermStore<T, V> termStore) {
        if (groundRule instanceof WeightedGroundRule) {
            ...
            OnlineHyperplane<V> onlineHyperplane = processOnlineHyperplane(function, (OnlineStreamingTermStore)termStore);
            if (onlineHyperplane == null) {
                return null;
            }

            // Non-negative functions have a hinge.
            return createLossTerm(termStore, function.isNonNegative(), function.isSquared(), groundRule, onlineHyperplane);
            ...
            onlineHyperplane.setConstant((float)(constraint.getValue() + onlineHyperplane.getConstant()));
            return createLinearConstraintTerm(termStore, groundRule, onlineHyperplane, constraint.getComparator());
        } else {
    ...
    private OnlineHyperplane<V> processOnlineHyperplane(GeneralFunction sum, OnlineStreamingTermStore<T, V> termStore) {
        ...
        for (int i = 0; i < sum.size(); i++) {
            ...
            if (term instanceof RandomVariableAtom) {
                ...
                int localIndex = onlineHyperplane.indexOfVariable(variable);
                if (localIndex != -1) {
                    ...
                    onlineHyperplane.appendCoefficient(localIndex, coefficient);
                } else {
                    onlineHyperplane.addTerm(variable, coefficient);
                }
            } else if (term.isConstant()) {
                // Subtracts because hyperplane is stored as coeffs^T * x = constant.
                onlineHyperplane.setConstant(onlineHyperplane.getConstant() - (float)(coefficient * term.getValue()));
            } else {
     ...
     */
    public abstract T createLossTerm(TermStore<T, V> termStore, boolean isHinge, boolean isSquared, GroundRule groundRule, OnlineHyperplane<V> hyperplane);

    /**
     * Create a hard constraint term,
     */
    public abstract T createLinearConstraintTerm(TermStore<T, V> termStore, GroundRule groundRule, OnlineHyperplane<V> onlineHyperplane, FunctionComparator comparator);
}
