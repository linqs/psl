...
public class DualBCDReasoner extends Reasoner<DualLCQPObjectiveTerm> {
    ...
    private double computePrimalVariableComponentRegularization(TermStore<DualLCQPObjectiveTerm> termStore, int componentIndex) {
        AtomStore atomStore = termStore.getDatabase().getAtomStore();
        ArrayList<GroundAtom> connectedComponentAtoms = atomStore.getConnectedComponentAtoms(componentIndex);
        float[] atomValues = atomStore.getAtomValues();
    ...
    protected double parallelComputeComponentDualObjectiveAndGradientNorm(DualLCQPTermStore termStore, int componentIndex, ObjectiveResult objectiveResult) {
        HashMap<Integer, ArrayList<DualLCQPObjectiveTerm>> connectedComponents = ((SimpleTermStore)termStore).getConnectedComponents();
        ArrayList<DualLCQPObjectiveTerm> component = connectedComponents.get(componentIndex);
